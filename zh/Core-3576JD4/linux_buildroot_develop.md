...
![](../../img/linux_buildroot_menuconfig.png)

我们可以在配置界面添加或裁剪一些工具，按需求定制系统功能。以添加 `qt53d` 为例：

输入 `/` 进入搜索界面，输入要查找的内容 `qt53d`，按回车进行搜索：

![](../../img/linux_buildroot_serach_qt53d.png)

![](../../img/linux_buildroot_qt53d.png)

选择 `1` 跳转到对应页面，按空格选中配置：

![](../../img/linux_buildroot_select_qt53d.png)

配置完成后，移动到 `Save` 按回车保存到 `.config`；移动到 `Exit` 按回车退出。

![](../../img/linux_buildroot_save_exit.png)
...
![](../../img/linux_buildroot_busybox_menuconfig.png)

配置完成后，移动到 `Exit` 按回车退出，在弹窗页面选择 `Yes` 保存到 `.config`。

![](../../img/linux_buildroot_busybox_save.png)
