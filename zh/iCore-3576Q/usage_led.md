...
标准的 Linux 专门为 LED 设备定义了 LED 子系统。 在 AIO-3576Q开发板中的LED 均以设备的形式被定义。用户可以通过 `/sys/class/leds/` 目录控制LED。
...
在 `kernel/arch/arm64/boot/dts/rockchip/rk3576-firefly-aio-3576q.dtsi` 文件中定义 LED 节点，可以用 pwm 控制也可以用 gpio 控制，类似这样：
```
#if LED_GPIO_OR_PWM
	firefly_leds: leds {
		compatible = "pwm-leds";
