...
AIO-3576JD4 has one green indicator LED, uncontrollable for software.

Another yellow LED is controllable by extended gpio PCA9555
...
Linux has its own LED subsystem for LED devices. In Core-3576JD4, LEDs are configured as LED class devices.You can control them via `/sys/class/leds/`.

The default status of the three on-board leds are:

* Green: Indicator LED, uncontrollable.
...
Defined LED node in file `kernel/arch/arm64/boot/dts/rockchip/rk3576-firefly-aio-3576jd4.dtsi`, like this:
```
#if LED_GPIO_OR_PWM
	firefly_leds: leds {
		compatible = "pwm-leds";
