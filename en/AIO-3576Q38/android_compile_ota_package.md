...
```
./FFTools/make.sh -d rk3576-firefly-aio-3576q38 -j8 -l rk3576_firefly_aio_3576q38-userdebug -o
```

#### Generate the full package path
```
out/target/product/rk3576_firefly_aio_3576q38/rk3576_firefly_aio_3576q38-ota-eng.xxx.zip 
```
#### Generate the base material package path
```
out/target/product/rk3576_firefly_aio_3576q38/obj/PACKAGING/target_files_intermediates/rk3576_firefly_aio_3576q38-target_files-eng.xxx.zip
```
Copy the base material package and rename it, mainly for subcontracting upgrades.
```
cp out/target/product/rk3576_firefly_aio_3576q38/obj/PACKAGING/target_files_intermediates/rk3576_firefly_aio_3576q38-target_files-eng.xxx.zip ~/ota/v1.zip 
```
...
```
In the `rockdev/Image-rk3576_firefly_aio_3576q38/` directory, find the corresponding firmware and burn it to the machine, then the machine can carry out the full package upgrade and the difference package upgrade,put 'update.zip' into /sdcard/ and restart the full package as prompted.
...
```
 cp out/target/product/rk3576_firefly_aio_3576q38/obj/PACKAGING/target_files_intermediates/rk3576_firefly_aio_3576q38-target_files-eng.xxx.zip  ~/ota/v2.zip
```
