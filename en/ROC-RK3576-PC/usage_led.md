...
There are 3 LEDs on the ROC-RK3576-PC development board, as the following table shows:
...
Linux has its own LED subsystem for LED devices. In ROC-RK3576-PC, LEDs are configured as LED class devices.You can control them via `/sys/class/leds/`.
...
Defined LED node in file `kernel/arch/arm64/boot/dts/rockchip/ROC-RK3576-PC.dts.dtsi`, like this:
```
#if LED_GPIO_OR_PWM
	firefly_leds: leds {
		compatible = "pwm-leds";
